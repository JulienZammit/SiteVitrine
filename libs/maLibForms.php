...
function mkInput($type,$name,$value="",$attrs="")
{
	...
	echo "<input class='form-control btn-others' $attrs type=\"$type\" name=\"$name\" value=\"$value\"/>\n";
}

function mkInputBoutton($type,$name,$value="",$attrs="")
{
	// Produit un champ formulaire
	echo "<input class='btn btn-header' $attrs type=\"$type\" name=\"$name\" value=\"$value\"/>\n";
}
...
{
	echo "<a class='dropdown-item'$attrs href=\"$url?$qs\">$label</a>\n";
}

function mkLienAdmin($url,$label, $qs="",$attrs="")
{
	echo "<a class='nav-link'$attrs href=\"$url?$qs\">$label</a>\n";
}
...
function mkLiens($tabData,$champLabel, $champCible, $urlBase=false, $nomCible="")
{
	...
	foreach ($tabData as $data) {
		echo '<li><a href="' ; 
		if ($urlBase) {
			echo $urlBase . "&" . $nomCible . "=" . $data[$champCible];
		} 
		else {
