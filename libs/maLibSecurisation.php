...
{
	// traitement auprès de la BDD => dans la couche modele 
	$id = verifUserBdd($login,$password);

	// verifUserBdd renvoie soit false, soit un id 
	// NB : un id ne peut etre nul 
	...
	$_SESSION["isAdmin"] = isAdmin($id,$login);
	$_SESSION["isBlacklisted"] = isBlacklisted($id);
